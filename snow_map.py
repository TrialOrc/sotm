...
class SnowMap:
    ...
    def initialize_snow_map(self):
        self.snow_map = noise_perlin[tcod.noise.grid(shape=(self.width, self.height), scale=0.25, indexing="xy")].transpose()  # create a noise grid
        self.snow_map = (self.snow_map + abs(self.snow_map.min()))
        self.snow_map = self.snow_map * (8 / self.snow_map.max())  # Normalize to 0.0 - 8.0
    ...
    def set_snow_levels(self, dungeon: GameMap, noise_map: np.ndarray):
        ...
        for tile_type, mask in ((tile_types.snow_low, low_mask), (tile_types.snow_med, med_mask),
                                (tile_types.snow_high, high_mask), (tile_types.snow_max, max_mask)):
